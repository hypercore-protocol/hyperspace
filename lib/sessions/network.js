...
const CONFIGURING_TIMEOUTS_SYMBOL = Symbol('configuring-network-timeouts')
...
module.exports = class NetworkSession {
  ...
  async configure ({ configuration: { discoveryKey, announce, lookup, remember }, flush, copyFrom, overwrite }) {
    ...
    let existing = await this._getConfiguration(discoveryKey)
    ...
    try {
      if (copyFrom) {
        let config = await this._getConfiguration(copyFrom)
