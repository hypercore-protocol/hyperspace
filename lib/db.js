...
const { NetworkConfiguration } = require('@hyperspace/rpc/messages')
...
module.exports = class HyperspaceDb extends Nanoresource {
  ...
  async putNetworkConfiguration (networkConfiguration) {
    ...
    return new Promise((resolve, reject) => {
      this._db.put(toDbKey(NETWORK_PREFIX, dkeyString), NetworkConfiguration.encode(networkConfiguration), err => {
        if (err) return reject(err)
        return resolve(null)
      })
    })
  ...
  async getNetworkConfiguration (discoveryKey) {
    ...
    return new Promise((resolve, reject) => {
      this._db.get(toDbKey(NETWORK_PREFIX, dkeyString), (err, node) => {
        if (err) return reject(err)
        return resolve(node && NetworkConfiguration.decode(node.value))
      })
    })
  }
  ...
  async listNetworkConfigurations () {
    return new Promise((resolve, reject) => {
      this._db.list(NETWORK_PREFIX, (err, nodes) => {
        if (err) return reject(err)
        return resolve(nodes.map(n => NetworkConfiguration.decode(n.value)))
      })
    })
  }
}
