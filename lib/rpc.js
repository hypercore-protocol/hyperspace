...
class HRPCServiceConfig {
  constructor (rpc) {
    const service = rpc.defineService({ id: 1 })

    this._set = service.defineMethod({
      id: 1,
      requestEncoding: messages.Config,
    ...
    this._get = service.defineMethod({
      id: 2,
      requestEncoding: messages.Config,
      responseEncoding: messages.Config
    })

    this._delete = service.defineMethod({
      id: 3,
      requestEncoding: messages.Config,
      responseEncoding: RPC.NULL
    })
  ...
  onRequest (context, handlers = context) {
    if (handlers.set) this._set.onrequest = handlers.set.bind(context)
    if (handlers.get) this._get.onrequest = handlers.get.bind(context)
    if (handlers.delete) this._delete.onrequest = handlers.delete.bind(context)
  }

  set (data) {
    return this._set.request(data)
  }

  setNoReply (data) {
    return this._set.requestNoReply(data)
  }
  ...
  deleteNoReply (data) {
    return this._delete.requestNoReply(data)
  }
}
...
module.exports = class HRPCSession extends HRPC {
  constructor (rawSocket, { maxSize = 2 * 1024 * 1024 * 1024 } = {}) {
    ...
    this.config = new HRPCServiceConfig(rpc)
