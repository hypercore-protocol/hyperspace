...
class HRPCServiceCorestore {
  ...
  onRequest (handlers) {
    if (handlers.open) this._open.onrequest = handlers.open.bind(handlers)
  }
...
class HRPCServiceHypercore {
  ...
  onRequest (handlers) {
    if (handlers.get) this._get.onrequest = handlers.get.bind(handlers)
    if (handlers.append) this._append.onrequest = handlers.append.bind(handlers)
    if (handlers.update) this._update.onrequest = handlers.update.bind(handlers)
    if (handlers.seek) this._seek.onrequest = handlers.seek.bind(handlers)
    if (handlers.has) this._has.onrequest = handlers.has.bind(handlers)
    if (handlers.download) this._download.onrequest = handlers.download.bind(handlers)
    if (handlers.undownload) this._undownload.onrequest = handlers.undownload.bind(handlers)
    if (handlers.close) this._close.onrequest = handlers.close.bind(handlers)
    if (handlers.onAppend) this._onAppend.onrequest = handlers.onAppend.bind(handlers)
  }
...
class HRPCServiceNetwork {
  ...
  onRequest (handlers) {
    if (handlers.configureNetwork) this._configureNetwork.onrequest = handlers.configureNetwork.bind(handlers)
    if (handlers.getNetworkConfiguration) this._getNetworkConfiguration.onrequest = handlers.getNetworkConfiguration.bind(handlers)
  }
